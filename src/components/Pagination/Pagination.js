...
export default class Pagination extends Component {
  selectPage = page => this.props.actions.goToPage(page);
  ...
  renderPagination = () => {
    ...
    return pages.map(number => {
      if (number <= 0 || number > total_pages) return null;
      return (
        <button
          className={current_page === number ? 'isActive' : null}
          key={uuid.v1()}
          onClick={() => this.props.actions.goToPage(number)}
        >
          {number}
        </button>
      );
